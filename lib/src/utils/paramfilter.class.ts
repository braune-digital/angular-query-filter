...
export class ParamFilter<E = Object> {
  ...
  public range: { total: number; pages: number; from?: number; to?: number } = {
    total: 0,
    pages: 0,
  };
  ...
  constructor(
    ...
    private headers: HttpHeaders | { [header: string]: string | string[] } = {} // important, do not delete!: init restore Service for static usage
  ) {}

  public refresh(): void {
    this.isLoadingEvent.next(true);
    this.refreshPromise().subscribe((response: Array<E>) => {
  ...
  preparePagination(response: any): void {
    if (response.headers.has("Content-Range")) {
      ...
      this.range["pages"] = Math.ceil(
        (this.range ? this.range.total : response.json().length) /
          this.resultsPerPage
      );
  ...
  public build(): any {
    ...
    if (
      this.filtersFromLastRequest &&
      this.filtersFromLastRequest !== filterObjectsString
    ) {
      this.page = 1;
    }
